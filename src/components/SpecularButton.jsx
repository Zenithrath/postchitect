  ...
  useEffect(() => {
    ...
    const update = (now) => {
      ...
      const steer =
        current.followMouse &&
        pointerAngle !== null &&
        (!current.autoAnimate || proximityT > 0);
      const target = steer ? pointerAngle : idleAngle;
      const diff =
        ((target - angle + Math.PI * 3) % (Math.PI * 2)) - Math.PI;
      angle += diff * (1 - Math.exp(-dt * 7));

      const brightTarget = current.autoAnimate ? 1 : proximityT;
      bright += (brightTarget - bright) * (1 - Math.exp(-dt * 8));

      lineC.set(current.lineColor);
      baseC.set(current.baseColor);
      program.uniforms.uAngle.value = angle;
      program.uniforms.uRadius.value =
        Math.min(current.radius, Math.min(sizeRef.w, sizeRef.h) / 2) * dpr;
